...
- `client.dart`: existing widget API; caller owns client; session in memory. Native uses configured HTTP(S) origin; web uses browser origin. No trusted native identity implied.
- `overlay.dart`: `NotetteFeedback` in MaterialApp builder above navigator; own Overlay ancestor for selection/tooltips. Drag stores fractional launcher position, survives routes/forms, resets on remount; safe-area/keyboard bounds +16px.
- Page URL = origin + current root-relative route. Logical-pixel viewport/screenshot; never synthesize DOM targets. No threads/triage/mentions/signup UI.
- Screenshot capture before dialog; unchecked opt-in/preview; upload after creation. Upload failure is partial success, never resend saved feedback.
