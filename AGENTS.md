...
8. **Separate documentation audiences.** READMEs are for end users: installation, configuration, usage and limitations only. ARCHITECTURE.md is LLM-only: terse, searchable file/symbol pointers, invariants, boundaries and non-obvious coupling. Minimize tokens; omit code-obvious facts, tutorials, history and duplicated rules. Do not add contributor, local-development, release or publishing guides to either.
